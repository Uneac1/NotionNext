...
import { useNavGlobal } from '@/themes/nav2'
  ...
  const handleSearch = async () => {
    try {
      ...
      if (!allNavPages || !Array.isArray(allNavPages)) {
        console.warn('allNavPages is not available')
        return
      }
      ...
      for (let i = filterAllNavPages.length - 1; i >= 0; i--) {
        ...
        if (hit) {
          // 高亮匹配的标题或摘要
          if (post.title && typeof post.title === 'string' && post.title.toLowerCase().indexOf(keyword.toLowerCase()) > -1) {
            post.highlightedTitle = highlightText(post.title, keyword)
          }
          if (post.summary && typeof post.summary === 'string' && post.summary.toLowerCase().indexOf(keyword.toLowerCase()) > -1) {
