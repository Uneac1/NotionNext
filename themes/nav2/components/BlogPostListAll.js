  ...
  const groupedArray = (filteredNavPages || [])?.reduce((groups, item) => {
    ...
    try {
      if (JSON.parse(siteConfig('NAV_AUTO_SORT', null, CONFIG))) {
        ...
        existingGroup = groups.length > 0 ? groups[groups.length - 1] : null // 获取最后一个分组
      }
    } catch (error) {
      console.warn('Error parsing NAV_AUTO_SORT config:', error)
      existingGroup = groups.length > 0 ? groups[groups.length - 1] : null // 默认获取最后一个分组
    }
