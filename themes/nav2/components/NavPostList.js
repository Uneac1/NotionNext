...
const NavPostList = (props) => {
  ...
  // 如果有搜索结果，显示搜索结果
  if (filteredNavPages && Array.isArray(filteredNavPages) && filteredNavPages.length > 0) {
    return (
      <div className='space-y-2'>
        ...
        {filteredNavPages.map((post, index) => {
          ...
          return (
            <div key={post?.id || index} className='border-l-2 border-green-500 pl-3'>
  ...
  if (!categoryOptions || !Array.isArray(categoryOptions)) {
    ...
    return <div id='category-list' className='dark:border-gray-700 flex flex-wrap  mx-4'>
            {categoryOptions.map(category => {
              if (!category || !category.name || typeof category.name !== 'string') return null
              ...
              return (
                    <Link
                        ...
                        <div> <i className={`mr-2 fas ${selected ? 'fa-folder-open' : 'fa-folder'}`} />{category.name}({category.count || 0})</div>
