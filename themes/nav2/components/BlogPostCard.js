...
const BlogPostCard = ({ post, className }) => {
  ...
  pageIcon =
    post.pageIcon && post.pageIcon.indexOf('amazonaws.com') !== -1
        ...
        <div className='stack-entry w-full flex space-x-3 select-none dark:text-neutral-200'>
          ...
          <div className='stack-comment flex-auto'>
            ...
            <p className='description font-normal'>
              {post.highlightedSummary ? (
                <span dangerouslySetInnerHTML={{ __html: post.highlightedSummary }} />
              ) : (
            ...
            {post.matchedContent && (
              <div className='mt-2 p-2 bg-gray-50 dark:bg-gray-700 rounded text-xs text-gray-600 dark:text-gray-300'>
                <div className='font-semibold text-gray-700 dark:text-gray-200 mb-1'>匹配内容：</div>
                <div 
                  className='line-clamp-3'
                  dangerouslySetInnerHTML={{ __html: post.matchedContent }}
                />
              </div>
            )}
          </div>
